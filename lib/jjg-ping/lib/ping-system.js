...
exports.ping = function ping(address, callback) {
    ...
    // Windows.
        ...
        process = cp.spawn('./lib/ping.exe', ['-n', '1', '-w', '5000', address]);
        result_line = 1;
        time_param = 4;
    ...
    process.stdout.on('data', function (data) {
        ...
        var lines = data.toString().split(endtype);
        ...
        // If it is present, find the latency value in ms.
            ...
            host = array[2].replace('PING', '', array[time_param]);
            // console.log('host ' + host);

            // If on a windows machine, also remove the 'ms'.
            if (platform == 'windows') {
              latency = latency.replace('ms', '', latency);
              host = array[2].replace('Pinging', '', array[time_param]);
            }
